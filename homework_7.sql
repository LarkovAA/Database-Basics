...
CREATE TABLE flights(
	...
	from_flights Varchar(145) NOT NULL,
	to_flights Varchar(145) NOT NULL
)
...
INSERT INTO flights (from_flights, to_flights) VALUES
('Moscow', 'Samara'),
...
SELECT
	id AS flight_id,
	(SELECT name FROM cities WHERE label = from_flights) AS `from`,
	(SELECT name FROM cities WHERE label = to_flights) AS `to`
FROM
	flights
ORDER BY
	flight_id;

-- Я не понимаю почему через join не получается сделать ........
...
FROM 
	flights f 
	JOIN cities c2 ON c2.label = f.from_flights OR c2.label = f.to_flights
